...
rejects that claim and cites the evidence delta. Oskar may accept the corrected
...
Before fan-out, Oskar and Hipson convert the task requirements into acceptance
...
only optimizer repair in their existing threads. Kakashi and Oskar then rerun
the failed checks and regression checks.

If all critical and major criteria pass, Oskar returns `stop_sufficient`. If
...
| Unsupported Oskar claim | Synthesis adds a critical or major claim without traceable evidence | Verify, demote, or reject it before delivery. |
...
- whether Oskar's final claims are traceable to revisions and evidence
